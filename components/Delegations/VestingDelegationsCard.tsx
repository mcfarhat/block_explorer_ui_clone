...
type VestingDelegationsCardProps = {
...
const VestingDelegationsCard: React.FC<VestingDelegationsCardProps> = ({
  delegatorAccount,
  startAccount,
  limit,
}) => {
  ...
  if (isVestingDelegationsError) {
    return <div>Error loading vesting delegations.</div>;
  }

  const delegations = vestingDelegationsData?.result || [];
  {console.log(delegations)}
  if (!delegations.length) return <div className="text-black">No vesting delegations found.</div>;
...
export default VestingDelegationsCard;
